...
try {
  bot.on('message', async (msg) => {
    ...
    if (msg.text == '/help') {
      ...
      if (fr.length != 0) {
        var frr = fr[1].toLowerCase()
        var request1 = require('request');
        if (df.file_size <= 2097152 && (frr == 'png' || frr == 'jpg' || frr == 'gif' || frr == 'heic')) {
          request1(options1, function (err, response, body) {
            if (!err && response.statusCode == 200) {
              var magigNumberInBody = body.toString('hex', 0, 4);
              if (magigNumberInBody == magic.jpg || magigNumberInBody == magic.png || magigNumberInBody == magic.gif || magigNumberInBody == magic.heic) {
